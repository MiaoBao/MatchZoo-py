...
- word weight is similarity between doc CLS and query token as weight
- add similarity between query CLS and doc token as weight
"""
import typing
...
class BertKNRMex(BaseModel):
    ...
    def forward(self, inputs):
        ...
        doc_token_d_weight = torch.squeeze(torch.einsum('b1d,brd->b1r',
            F.normalize(bert_d[:,0:1], p=2, dim=-1),
            F.normalize(bert_d, p=2, dim=-1)
        ), 1)
        
        # sim(doc token, query CLS) dim = [B,R]
        doc_token_q_weight = torch.squeeze(matching_matrix[:,0,1:], 1)
        ...
        word_weight = F.softmax(torch.squeeze(matching_matrix[:,1:,0], -1), dim = 1)
        
        weighted_word_score = word_weight * word_score
